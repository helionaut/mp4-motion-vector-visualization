...
- Known-good private/user-data baseline: not established yet; HEL-152 now has a committed private staging path (`scripts/prepare_private_inputs.sh` plus `configs/input_sets/private-template.json`) and can execute the published public runner on this host, but it should not rerun on private data until the real user MP4 pair is staged
